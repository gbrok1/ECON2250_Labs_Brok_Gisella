...
Unemployed_individuals <- .2*Unemployment_Rate*Population
Unemployed_individuals
```
...
Q_GDP_growth_rate<- c(.018,.02,.0175,.019)
Q_GDP_growth_rate
...
**Sample covariance:** S$s_{xy}=\frac{1}{n-1}/sum_{i=1}^n(x_i-\overline{x})(y_i-\overline{y})$

**Sample correlation:** $$r = \frac{ \sum_{i=1}^{n} (x_i - \bar{x})(y_i - \bar{y}) }{ \sqrt{ \sum_{i=1}^{n} (x_i - \bar{x})^2 } \cdot \sqrt{ \sum_{i=1}^{n} (y_i - \bar{y})^2 } }$$
